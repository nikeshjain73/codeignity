...
const TeamSection = () => (
    <section id="team" className="py-5 py-lg-5" style={{ 
        ...
        <Container>
            ...
            <Row className="row-cols-2 row-cols-md-3 row-cols-lg-5 g-4 justify-content-center">
                {teamMembers.map((member, index) => (
                    <Col key={index} className="text-center fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                        <div className="team-avatar-wrapper">
                            <img src={member.img} alt={member.name} className="team-avatar" />
                        </div>
                        <h5 className="fw-bold mt-3 mb-2" style={{ 
                        ...
                        <p className="text-dark small mb-1 fw-medium" style={{ opacity: 0.7 }}>{member.role}</p>
                        <p className="text-dark small" style={{ fontSize: '0.85rem', opacity: 0.6 }}>{member.expertise}</p>
                    </Col>
                ))}
            </Row>
        </Container>
